...
import { signalEventRefreshConfig } from './signal-event-config';
...
export async function ensureSignalEventStorage() {
  ...
  signalEventStorageReady = (async () => {
    await assertInvestmentRadarTablesReady(['signal_event', 'signal_evidence']);
  })().catch((error) => {
...
export async function refreshSignalEventsFromExternalLeads() {
  ...
  for (const lead of leads) {
    ...
    await prismaClient.$executeRawUnsafe(
      ...
      lead.sourceType || signalEventRefreshConfig.sourceType,
      lead.sourceName || '',
      lead.sourceUrl,
      ...
      JSON.stringify({
        ...
        pipeline: signalEventRefreshConfig.description,
      }),
    );
