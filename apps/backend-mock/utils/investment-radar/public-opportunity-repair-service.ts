...
async function getRepairCandidates(limit: number) {
  ...
  return prismaClient.$queryRawUnsafe<RepairCandidateRow[]>(
    `
      SELECT
        audit_scope.opportunityId,
        audit_scope.opportunityType,
        audit_scope.sourceSite,
        audit_scope.sourceUrl,
        audit_scope.sourceKey,
        audit_scope.sourceTable,
        audit_scope.sourceId,
        audit_scope.title,
        audit_scope.city,
        audit_scope.district,
        audit_scope.areaText,
        audit_scope.publishedAt,
        audit_scope.publishedDateText,
        audit_scope.opportunityStatus,
        audit_scope.score,
        audit_scope.descriptionPreview,
        audit_scope.detailJsonPreview,
        CAST(opo.detail_json AS CHAR) AS detailJson,
        audit_scope.lastSyncedAt,
        audit_scope.createTime,
        audit_scope.updateTime,
        audit_scope.isGuangdong,
        audit_scope.isEffective,
        audit_scope.missingSourceUrl,
        audit_scope.missingSupplyLocation,
        audit_scope.missingPublishedAt,
        audit_scope.suspiciousPublishedAt,
        audit_scope.missingHashOrDetailEvidence,
        audit_scope.proposedDowngradeStatus
      FROM (${auditScopeSql}) audit_scope
