import { submitReleaseContactRestriction } from '~/utils/investment-radar/contact-restriction-service';
import {
...
export default eventHandler(async (event) => {
  ...
  try {
    ...
    const result = await runWithRadarSharedScope(async () => {
      return await submitReleaseContactRestriction({
    ...
    console.error('submit contact restriction release failed:', error);
    return serverErrorResponse('提交解除申请失败', event);
  }
});
