import { rejectReleaseContactRestriction } from '~/utils/investment-radar/contact-restriction-service';
import {
...
export default eventHandler(async (event) => {
  ...
  try {
    ...
    const result = await runWithRadarSharedScope(async () => {
      return await rejectReleaseContactRestriction({
    ...
    console.error('reject contact restriction release failed:', error);
    return serverErrorResponse('驳回解除限制失败', event);
  }
});
