...
import { reclaimStaleRunningCrawlerTaskItems } from '~/utils/investment-radar/crawler-task-item-repository';
...
export default eventHandler(async (event) => {
  ...
  try {
    const result = await runWithRadarSharedScope(async () => {
      ...
      const staleMinutes =
        body.staleMinutes === undefined ? undefined : Number(body.staleMinutes);
      const reclaimResult = await reclaimStaleRunningCrawlerTaskItems({
    ...
    return serverErrorResponse('回收超时采集 URL 失败', event);
  }
});
