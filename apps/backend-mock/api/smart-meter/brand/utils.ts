...
export function normalizeMeterBrandData(body: Record<string, any>) {
  const data = { ...body };
  ...
  if (data.meterType !== undefined) {
    data.meterType = String(data.meterType || '').trim();
  }
...
export async function ensureSingleDefaultMeterBrand(params: {
  excludeId?: number;
  meterType: string;
  prismaClient: any;
}) {
  await params.prismaClient.meterBrand.updateMany({
    data: {
      isDefault: false,
    },
    where: {
      meterBrandId: params.excludeId
        ? {
