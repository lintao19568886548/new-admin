...
export default eventHandler(async (event) => {
  ...
  try {
    ...
    if (query.meterType) {
      where.meterType = normalizeOptionValue(query.meterType);
    }
    if (keyword) {
      where[field] = {
        contains: keyword,
    ...
    const records = await prismaClient.meterBrand.findMany({
    ...
    console.error('获取水电表品牌搜索候选失败:', error);
    return serverErrorResponse('获取水电表品牌搜索候选失败', event);
  }
});
