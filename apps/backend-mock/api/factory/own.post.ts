...
export default eventHandler(async (event) => {
  ...
  // factoryData 包含 parkId, factoryName 等基本信息，自动设置 isOwn = true
  const { floors, ...factoryData } = body;
  console.log('floors', floors);
  console.log('创建自有厂房，factoryData:', factoryData);
  ...
  try {
    ...
    console.error('创建自有厂房及关联数据失败:', error);
    // 提供更详细的错误信息给前端可能有助于调试，但生产环境要注意信息安全
    return serverErrorResponse(
      `创建自有厂房失败: ${error.message || '未知错误'}`,
      event,
    );
  }
});
