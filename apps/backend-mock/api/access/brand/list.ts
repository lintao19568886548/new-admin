...
export default eventHandler(async (event) => {
  ...
  try {
    ...
    const total = await prismaClient.accessBrand.count({ where });
    const items = await prismaClient.accessBrand.findMany({
    ...
    console.error('获取门禁品牌列表失败:', error);
    return serverErrorResponse('获取门禁品牌列表失败', event);
  }
});
