...
function findNodeByValue(value?: string) {
  ...
  const stack = Array.isArray(meterTreeRaw.value)
    ? [...meterTreeRaw.value]
    : [];
  while (stack.length > 0) {
    const n = stack.pop();
...
<template>
  <div class="mobile-meter-container">
    ...
    <Spin :spinning="loading" :tip="$t('加载中...')">
      <div v-if="list.length > 0" class="meter-list">
        <Card
          v-for="item in list"
          :key="
        ...
        >
          ...
          <div class="card-content">
            <div class="amount-display">
              <span class="amount-label">总用电量(度)</span>
              <span class="amount">{{ item.dataValue }}</span>
            </div>

            <div class="kv-grid">
              <div class="kv-item">
                <span class="kv-label">尖</span>
                <span class="kv-value">{{ item.dataValue1 ?? '-' }}</span>
              </div>
              <div class="kv-item">
                <span class="kv-label">峰</span>
                <span class="kv-value">{{ item.dataValue2 ?? '-' }}</span>
              </div>
              <div class="kv-item">
                <span class="kv-label">平</span>
                <span class="kv-value">{{ item.dataValue3 ?? '-' }}</span>
              </div>
              <div class="kv-item">
                <span class="kv-label">谷</span>
                <span class="kv-value">{{ item.dataValue4 ?? '-' }}</span>
              </div>
            </div>
            ...
            <div class="info-grid">
              ...
              <div class="info-item">
                <span class="info-label">写入时间</span>
                <span class="info-value">{{
                  formatDateTime(item.writeTime)
                }}</span>
              </div>
            </div>
          </div>
