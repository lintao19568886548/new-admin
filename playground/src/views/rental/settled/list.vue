...
const [Grid, gridApi] = useVbenVxeGrid({
  ...
  gridOptions: {
      ...
      ajax: {
        query: async ({ page }) => {
          try {
            ...
            console.error('获取入驻厂房列表失败:', error);
            message.error('获取入驻厂房列表失败');
            return {
    ...
    rowConfig: {
      keyField: 'factoryId', // 使用厂房ID作为主键字段
    },
    toolbarConfig: {
      custom: true,
      export: false,
...
<template>
  <Page auto-content-height>
    <FormModal @success="onFormSuccess" @refresh="refreshGrid" />
    <Grid :table-title="$t('page.rental.settled')">
