...
const record = computed<WaterItem>(() => modalApi.getData() || {});
...
const tableData = computed(() => {
  ...
  const actualUsage1 = monthlyUsage1 * multiplier1; // 本月实际用水量 = 本月用水量 * 倍数
  const unitPrice1 = record.value.unitPrice || 0;
  const amount1 = actualUsage1 * unitPrice1; // 水费金额 = 本月实际用水量 * 单价

  // 第二行示例数据
  const lastMonthReading2 = 200;
  const currentMonthReading2 = 240;
  const monthlyUsage2 = currentMonthReading2 - lastMonthReading2;
  const multiplier2 = 1.2;
  const actualUsage2 = monthlyUsage2 * multiplier2; // 本月实际用水量 = 本月用水量 * 倍数
  const unitPrice2 = 5.5;
  const amount2 = actualUsage2 * unitPrice2; // 水费金额 = 本月实际用水量 * 单价
  ...
  return [
    ...
    // 合计行（只计算本月实际用水量和水费金额）
    {
      actualUsage: totalActualUsage, // 本月实际用水量合计
      amount: totalAmount, // 水费金额合计
