...
import type { AccessBrandOptionField } from '#/api/access/brand';

import { formatDateTime } from '@vben/utils';

import { z } from '#/adapter/form';
import { getAccessBrandSearchOptions } from '#/api/access/brand';

export const ACCESS_BRAND_ENABLED_OPTIONS = [
  { label: '启用', value: true },
  { label: '停用', value: false },
];

export const ACCESS_BRAND_DEFAULT_OPTIONS = [
  { label: '当前选用', value: true },
  { label: '备选', value: false },
];

function createBrandSearchSchema(
  fieldName: AccessBrandOptionField,
  label: string,
): VbenFormSchema {
  return {
    ...
    componentProps: {
      allowClear: true,
      alwaysLoad: true,
      api: getAccessBrandSearchOptions,
      class: 'w-full',
...
export function useFormSchema(): VbenFormSchema[] {
  return [
    ...
    },
    {
      ...
      componentProps: {
        buttonStyle: 'solid',
        options: ACCESS_BRAND_ENABLED_OPTIONS,
        optionType: 'button',
      },
      defaultValue: true,
      fieldName: 'enabled',
    ...
    },
    {
      ...
      componentProps: {
        buttonStyle: 'solid',
        options: ACCESS_BRAND_DEFAULT_OPTIONS,
        optionType: 'button',
      },
      defaultValue: false,
      fieldName: 'isDefault',
...
export function useGridFormSchema(): VbenFormSchema[] {
  return [
    createBrandSearchSchema('brandName', '品牌名称'),
    createBrandSearchSchema('brandCode', '品牌编码'),
    createBrandSearchSchema('protocolType', '协议类型'),
    {
      component: 'Select',
      componentProps: {
        allowClear: true,
